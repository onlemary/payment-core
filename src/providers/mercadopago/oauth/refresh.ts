...
export async function refreshTokenWithLock(
  ...
  // Lock cleanup lives in a single `finally` so the lock is ALWAYS released,
  // regardless of the return path (success, non-OK response, or thrown error).
  // This prevents a future return path from leaking a permanently-held lock.
